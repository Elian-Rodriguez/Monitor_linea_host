...
def index_admin():
    if request.method == 'POST':
        ...
        if (usuario == "Homero" and password == "Linux-2020"):
            ...
            data_3=Tienda.listar_pos_server_caido()
            menssanges = 'Ingreso Autorizado'
            flash(menssanges)
            return render_template('index_admin.html', shops=data, offlines=data_2, CAIDA_POS_SERVER =data_3)
        else:
...
def listar_historial():
    data = Tienda.listar_historial()
    return render_template('listar_historial.html',historial=data)
